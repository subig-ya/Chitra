...
export default function PanelDashboardArtist() {
  const { user } = useAuth();
  ...
  const stats = [
    ...
    { label: "Pending payment", value: pending, view: "orders", icon: Icons.wallet },
    { label: "Escrow value", value: rs(escrow), view: "orders", icon: Icons.shield },
    { label: "Live artworks", value: liveArtworks, to: "/artworks/mine", icon: Icons.palette },
    { label: "Unread messages", value: unreadMessages, to: "/messages", icon: Icons.chat },
  ];
  ...
  const quickActions = [
    { label: "Add a new artwork", to: "/artworks/mine", icon: Icons.plus },
    { label: "Check commission requests", to: "/requests", icon: Icons.clipboard },
    { label: "Reply to messages", to: "/messages", icon: Icons.chat },
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="flex flex-wrap items-end justify-between gap-6">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-lg text-sm leading-relaxed text-ink-soft">
            Manage your artworks, commissions, messages and reports from your studio.
          </p>
        </div>
        <Link
          to="/artworks/mine"
              ...
              >
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-plum">
                    ...
                    <span className="text-ink-soft">{o.buyerId?.name || "Collector"}</span>
                  </p>
      ...
      {/* recent artworks + notifications + actions */}
      <div className="grid gap-8 lg:grid-cols-12">
        {/* recent artworks */}
        <section className="lg:col-span-5">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-display text-xl font-semibold text-plum">Recent artworks</h2>
            <Link
              to="/artworks/mine"
              className="inline-flex items-center gap-1 text-sm font-medium text-rose transition hover:text-plum"
            >
          ...
          {recentArtworks.length === 0 ? (
            <p className="rounded-xl border border-dashed border-plum/20 bg-ivory p-8 text-center text-sm text-ink-muted">
              No artworks yet. List your first artwork to start receiving commissions.
            </p>
          ) : (
            <div className="space-y-2">
              {recentArtworks.map((a) => (
                <Link
                  key={a._id}
                  to="/artworks/mine"
                  className="group flex items-center gap-3 rounded-xl border border-plum/10 bg-ivory p-3 transition hover:border-rose/40"
                >
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-lavender/70 text-plum-600">
                    {Icons.image}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-plum">{a.title}</span>
                    <span className="block text-xs text-ink-muted">
                      {a.price ? rs(a.price) : "Price TBD"} · {a.isVerified ? "Live" : "Draft"}
                    </span>
                  </span>
                </Link>
              ))}
